...
and two rounds of shift-and-add reduction) it reaches $1.59$~bytes/cycle of
bulk throughput on a 256\,KiB message and $133$~cycles per hash on short
...
the bulk throughput $9\times$ higher ($14.4$~bytes/cycle), but that variant is
...
polynomial rows use the framework implementations.}
...
\texttt{XXH3-128} (128-bit output)          & 1 & 28.5$^*$ & 33.8 & 24.0 \\
\texttt{MUM} v3                             & 1 &  0$^*$   & 33.0 & 26.8 \\
\texttt{komihash} v5.34                     & 1 & $\ge32^*$ & 24.8 & 24.3 \\
\hline
\multicolumn{5}{l}{\emph{proven, key $> 8$ words}} \\
ChainHash, 1\,KB blocks                     & 138 & 62 & 55.2 & 25.2 \\
ChainHash, 256\,B blocks                    &  42 & 62 & 51.2 & 27.0 \\
ChainHash, 64\,B blocks                     &  18 & 62 & 14.5 & 19.4 \\
...
(one \texttt{PMULL} per $16$ bytes into four $128$-bit accumulators) and its
...
sub-block; the byte length is XORed into the last $a_i$; and the result is
passed through a random monic polynomial of degree~$7$ evaluated with four
multiplications, $\lfloor7/2\rfloor+1$ as in \Cref{thm:main}, by a
characteristic-$2$ circuit whose coefficient map is a bijection of $\F^7$
(\Cref{sec:ph} gives the explicit decoder), so its seven parameters are drawn
uniformly at random and the output is $7$-wise independent.  Degree $7$ is the
least that suffices: over $\F_{2^{64}}$ the $\F_2$-degree of $v\mapsto v^e$ is
the number of ones in the binary expansion of $e$, so degree-$3$ and
degree-$5$ finalizers are quadratic over $\F_2$ and fail $23$ and $21$ of
the $188$ standard SMHasher3 tests~\cite{smhasher3} (Zeroes, Sparse,
Permutation, TwoBytes, Bitflip, SeedZeroes), whereas degree $7$ is the first
cubic option and passes; $S=2$
gives a one-block message a second message-dependent multiplication for the
same reason.  The collision probability is at most $(p+2)/2^{64}$ for two
...
tests of the full SMHasher3 suite, including a new seeded-differential test
...
With $1$\,KB blocks and a $138$-word key it runs at $55$\,GB/s on the M2 Pro,
above every heuristic hash in the table and above UMASH, with a proof, and the
rate is flat up to $64$\,MB messages; the block size is exactly the key-budget
knob, and at $64$-byte blocks, close to the one-cache-line budget, the hybrid
is slower than the fanned-out recurrence alone on long messages ($14.5$
against $23.8$\,GB/s at $16$\,KB).  The implementation and a
