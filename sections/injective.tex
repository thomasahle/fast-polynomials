...
ChainHash, 1\,KB blocks                     & 137 & 62 & 57.3 & 28.5 \\
ChainHash, 256\,B blocks                    &  41 & 62 & 51.0 & 31.1 \\
ChainHash, 64\,B blocks                     &  17 & 62 & 14.1 & 21.3 \\
...
sub-block; the byte length is XORed into the last $a_i$; one further key
...
explicit decoder), so its five parameters are drawn uniformly at random and
the output is $5$-wise independent, which is the independence that linear
probing provably needs~\cite{pagh2009linear,patracscu2016kindependence}.
...
quadratic in the bits of its input, with affine discrete derivatives, and the
fixed-seed keysets of SMHasher3~\cite{smhasher3} (Zeroes, Sparse,
Permutation, TwoBytes, Bitflip, SeedZeroes) detect exactly that: the
...
With $1$\,KB blocks and a $137$-word key it runs at $57$\,GB/s on the M2 Pro,
...
is slower than the fanned-out recurrence alone on long messages ($14.1$
