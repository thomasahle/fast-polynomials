...
Throughout this and the following appendix, a \emph{scalar} means a quantity independent of
...
\begin{definition}[Extractable]
    ...
    \[
        \alpha_i\polyfrom
        \bigl((\coeff{P}{j})_{\,j\in\idx{\deg P}}\bigr)
        \qquad(i\in\rng n).
    \]
    ...
    \[
        \alpha_i\polyfrom
        \Bigl((\coeff{B_k}{r})_{\,k\in[t],\,r\in\idx{\deg B_k}}\ ;\
              (\coeff{P}{j})_{\,j\in\idx{\deg P}}\Bigr)
        \qquad(i\in\rng n).
    \]
\end{definition}
...
\end{proof}
In every application, $D$ is the explicit decoder supplied by the coefficient
recurrences, not an inverse inferred from a determinant.  The lemma verifies
the other composition for that same supplied polynomial map.
...
\begin{definition}[Polynomially recoverable polynomial]
    ...
    \]
    Thus ``derivable'' in this and the following appendix is simply shorthand
...
\begin{figure}[htbp]
  \centering
  \begin{minipage}[b]{0.32\textwidth}\centering\input{figures/fig_jacobian_p7}\end{minipage}\hfill
  \begin{minipage}[b]{0.62\textwidth}\centering\input{figures/fig_jacobian_p15}\end{minipage}
  ...
  The patterns are drawn in characteristic zero: blue (dark) cells are nonzero constants, orange (light) cells are non-constant polynomials in the parameters, and white cells are zero.  Reduction to positive characteristic can change the pattern; for example, the slope-$6$ entries in the top two rows of $P_{15}$ vanish in characteristic~$3$, although its decoder still works there.
  Decoding proceeds top-down through fresh quantities, which may be parameters or internal values rather than individual columns of this Jacobian.
...
\begin{figure}[H]
  \centering
  \resizebox{\textwidth}{!}{\input{figures/fig_triangular_blocks}}
  \caption{The two structural moves behind the stage tables (styles as
  keyed later in \Cref{fig:decoder-language} of \Cref{appendix:constructions}).
...
\begin{array}{c|c|c}
  ...
  \text{square }(\operatorname{char}\mathbb F\ne2)&n+G
    &\coeff{\Psi}{n+i}=2\coeff{\Phi}{i}+F_i
\end{array}
\]
In the square row, $\Phi=xP^{(1)}+P^{(2)}$,
$\Psi=x(P^{(1)})^2+(P^{(2)})^2$, and $F_i$ is determined by already
recovered higher rows (including the fixed correction $F_n=-1$).
...
\begin{figure}[H]
  \centering
  \resizebox{\textwidth}{!}{\input{figures/fig_causal_closures}}
  ...
  the common degree and exposes the old row with slope~$2$ when
...
\begin{figure}[H]
  \centering
  \resizebox{\textwidth}{!}{\input{figures/fig_top_window_calculus}}
  ...
  pivot~$m$, required to be nonzero in the field; a lower-degree perturbation changes only the explicitly marked
  boundary row; and the square gadget is decoded as three consecutive
  shells (in characteristic different from~$2$).  In particular, panels (a)--(c) compose into the relative shell
