...
\subsubsection{Degree \texorpdfstring{$3$}{3}}
...
\subsubsection{Degree \texorpdfstring{$7$}{7}: direct septic base}
...
\begin{figure}[H]
  \centering
  \resizebox{0.95\textwidth}{!}{\input{figures/fig_septic_circuit}}
  \caption{The four-multiplication circuit for the septic base $P_7$ of \Cref{lem:septic-base}.
  Every multiplication gate takes two affine combinations of $x$ and previously computed values.  The six parameters $\alpha_1,\ldots,\alpha_6$ enter as constant terms of these gate inputs; $\alpha_0$ is added at the output.}
...
\subsubsection{Degree \texorpdfstring{$15$}{15}}
\begin{algorithm}[H]
    \caption{
...
\subsubsection{Degree \texorpdfstring{$27$}{27}}
\begin{algorithm}[H]
    \caption{
...
\subsubsection{Degree \texorpdfstring{$31$}{31}}
\begin{algorithm}[H]
    \caption{
...
\begin{figure}[H]
  \centering
  \resizebox{\textwidth}{!}{\input{figures/fig_special_case_decoders}}
  \caption{The three finite decoders in one visual template.  A coloured
  block denotes a recovered polynomial or terminal scalar data; square
  windows expose the monic factors, while the terminal blocks use the
  displayed scalar pivots or, for $C$ at degree $31$, division by $x$.
  Red cells record boundary contributions from the next block that are
  already known when subtracted.  For degree $15$, the two corrections
  beside the $H_4$ block occur in rows $5$ and $4$; the value of $b$ is
  recovered in row $7$.  Recovering a polynomial and decoding its internal parameters
...
\begin{proof}
    ...
    \end{array}
    \]
    At a shared boundary, subtract the displayed known contribution from
    the next block.  Most are monic coefficients $1$ or $-1$ (including
    the row-$4$ correction for $xC$ in the $31$ case).  For degree $15$,
    the $H_4$ window also includes the contributions $-1$ in row $5$ and
    $-(2b+2)$ in row $4$ from $-(x+1)U^2-V^2$; the preceding row-$7$
    pivot has already recovered $b$, so both corrections respect the cutoff.
