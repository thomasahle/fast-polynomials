\subsection{The $T$ recursion and the $4k+1$ family}
...
\begin{figure}[H]
  \centering
  \resizebox{\textwidth}{!}{\input{figures/fig_T_tower}}
...
\begin{proof}
    ...
    \Cref{lem:fill-Q-count}, and the two new degree-$2^{l+1}$ powers cost one
    product each.  Thus in both cases the even recurrence is
    \[
...
\begin{lemma}\label{lem:Rk2l}
    ...
    \begin{enumerate}
        ...
        \Cref{def:coefficient-triangular}.  Hence the parameters
        ...
        branch.  With $H_1=H_D$, $H_2=\widetilde H_D$,
        $\sigma_i=[x^{D/2-1}]S^{(i)}_1$, and
        $h_i=[x^{D-1}]H_i$, one has
        \[
...
\begin{figure}[H]
  \centering
  \resizebox{\textwidth}{!}{\input{figures/fig_Rk2l_stages}}
  ...
  low unitriangular $Q$-block is read.  The red markers are the
  parameter-free boundary rows that must be subtracted before crossing into
  the next segment.}
...
\begin{proof}
    ...
    block and append the low unitriangular block.  On a first reading, one may
    follow the displayed identity, stage table, and final concatenation in each
    branch.  The paragraphs headed ``seam rows'' justify precisely the red
    boundary markers in the figure.  \Cref{alg:decode-Rk2l} restates the same
    ...
    indices in the shared-base formulas are defined.
      ...
      $\sigma$ are scalars.  In the formulas below $H_1=H_D$ and
      $H_2=\widetilde H_D$, and
      ...
      \[
        ...
        \tag{R-even-block}
      \]
      where the $\eta$'s are the coefficient parameters of the two
      $Q$-polynomials.
      The recursive definition and the binomial theorem give the single
      identity
      \[
        R^{(i)}_{k,D}=mE_iH_i^{k-2}+\widehat R_i+R'_i,
        \tag{R-even-exp}
      \]
      with $\deg\widehat R_i,\deg R'_i\le b$.  Explicitly,
      $\widehat R_i=\sum_{q=2}^{m}\binom mq E_i^qH_i^{k-2q}$;
      terms with $q\ge3$ have degree at most $b-D$.

      The summand $-m(S^{(i)}_1)^2H_i^{k-2}$ has degree $d$ and
      ...
      $h_i=[x^{D-1}]H_i$.  This proves assertions~(2) and~(4) for the
      even branch.
      ...
      \begin{array}{c|c|c|c}
        ...
        b+D-1,\ldots,b+r+1
          &\alpha_{b+r+1},\ldots,\alpha_{b+D-1}&-2m=-k&-xU^2H_1^{k-2}\\
        b+r&\alpha_{b+r}=\delta&-2m=-k&-\widetilde U^2H_2^{k-2}\\
        b+r-1,\ldots,b+1
          &\alpha_{b+1},\ldots,\alpha_{b+r-1}&m&xVH_1^{k-2}\\
        b&\alpha_b=\sigma&m&\sigma H_2^{k-2}\\
        b-1,\ldots,0&\text{recursive block}&\text{inner pivot}&R'_1,R'_2
      \end{array}
      \tag{R-even-table}
      \]
      For example, a coefficient $\eta^+_s$ first occurs as
      $2\eta^+_s$ in $U^2$ at degree $r+s$; the $x$-shift and the monic
      factor $H_1^{k-2}$ put it in row $b+r+s+1$, with slope $-2m$.
      ...
      Let $s=[x^{r-1}]U$; here $h=[x^{D-1}]H_1$ is the value in
      ...
      \begin{itemize}
        ...
        $\binom m2E_i^2H_i^{k-4}$ (for $m=1$ the sum $\widehat R_i$ is
        empty and $\binom m2=0$), with
        $(A_i,B_i,f,e)=\bigl(H_i^{k-4},\binom m2E_i^2,(k-4)D,2D\bigr)$:
      ...
      $(H_1,U_1)=(H,U)$ and $(H_2,U_2)=(\widetilde H,\widetilde U)$, set
      \[
      \begin{aligned}
      E_i^{U}&\coloneqq
        L_i(H_i+U_i)^{k-1}-H_i^k+cU_i^2H_i^{k-2},\\
      E_i^{G}&\coloneqq
      ...
      \Cref{lem:odd-T-cubic-loss}, applied to $(H_i,U_i)$, gives the first
      ...
      $-cU_i^2H_i^{k-2}$ has degree $d$ and leading coefficient $-c$;
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}
    \Require integers $l\ge 2$, $k\ge 1$, known powers $(H_2,\ldots,H_{2^l})$ and the shifted power $\tilde H_{2^l}$, and $P=xR^{(1)}_{k,2^l}+R^{(2)}_{k,2^l}$; if $l=2$ and $k\ge3$ is odd, require $\tilde H_4-H_4$ to have degree zero in $x$
...
\begin{lemma}[Leading coefficients of the remainder polynomials]\label{lem:Rk2l-leading-coeff}
    ...
    \[
      ...
      h_i\coloneqq [x^{D-1}]H_i,
    \]
    where $H_1=H_D$ and $H_2=\widetilde H_D$.  Then, for $i=1,2$,
    \[
      [x^d]R^{(i)}_{k,D}=-\gamma_k,
      \qquad
      [x^{d-1}]R^{(i)}_{k,D}
...
\begin{proof}
    ...
    the top two coefficients of $H_i$ and $S^{(i)}_1$.  In every even-$k$
...
\Cref{fig:4k1-crown} pictures the five-pivot crown over the $T_{2k,2}$ call.

\begin{figure}[H]
  \centering
  \resizebox{\textwidth}{!}{\input{figures/fig_4k1_crown}}
  \caption{The $4k+1$ construction and its inverse crown.  Forward, the
  quadratic and quartic powers are shared by the two branches of the
