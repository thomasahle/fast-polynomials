...
they assemble into \Cref{alg:final-decoder}.
The paper and Lean use the same binary known-powers construction;
\Cref{thm:construction-height} records its complete logarithmic-height bound.
\Cref{fig:final-recursion} charts the routing of the strong induction.

\begin{figure}[H]
  \centering
  \resizebox{\textwidth}{!}{\input{figures/fig_final_recursion}}
  ...
  branches use \Cref{alg:constr-known-2n-1}
  (\Cref{thm:construction-height}), giving height
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}
    ...
    \Else
      \State Write $n=8k+7$ with $k\ge 2$ and extend the smaller compatible joint realization for $2k+1$ by \Cref{alg:constr-8k+7}, using $\mathcal Q_{2k+1}$ and $\mathcal Q_{4k+3}$ (barred when their degree is $7\pmod8$ and at least $15$).
    \EndIf
  \end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}
    ...
    \ElsIf{$n\equiv 3\pmod 8$}
      ...
      \State Decode $S^{(1)}_2=Q_{4k+1}$ first via \Cref{lem:Q4k+1-from-H2}, which reconstructs $H_4$; for $k>1$, only then decode $S^{(1)}_3=\mathcal Q_{2k-1}$ by the dispatch of \Cref{lem:odd-gadgets-H2H4}, with \Cref{alg:decode-Q-2kminus1} for nested Mersenne blocks.  For $k=1$, read $S^{(1)}_3=\alpha_1$ directly.
    \Else
...
\begin{proof}
    ...
    of \Cref{thm:construction-height} threaded through the same
    ...
    smaller joint circuit once and reuse its recorded $H_2$ wire.  In the
    $8k+3$ step, obtain $H_4$ from the $Q_{4k+1}(x,H_2)$ call; in the
    $8k+7$ step, reuse the smaller pair's recorded $H_4$ wire.  Use these
    byproducts in the auxiliary gadgets, and then form the two displayed
    difference-of-squares outputs.  Each difference of squares uses one
    product: $(S^{(i)}_3+S^{(i)}_2)(S^{(i)}_3-S^{(i)}_2)$ in the $8k+7$
    step, and $(S^{(i)}_2+S^{(i)}_1)(S^{(i)}_2-S^{(i)}_1)$ in the $8k+3$
    step, for $i=1,2$, as displayed in the respective algorithms.
