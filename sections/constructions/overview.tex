...
adds one product), using the binary known-powers recursion of
...
height; the separate addition-accounting appendix proves the two stated
addition bounds for the same polynomial family.
...
recovery and its causal version.  The binary known-powers construction of
\Cref{sec:peeled-Q} supplies the gadgets $Q_{2^k-1}$, with height at most
$k$, and the fill construction uses these gadgets to extend a compatible
pair.  The central $T_{k,2^l}$ lemma proves one
...
\smallskip\noindent\textbf{Fill $A_{2^l}$.}
Attach monic heads above and below a pair; decode upper head
$\to$ shifted input window $\to$ lower head.  The heads contain
binary known-powers $Q$-gadgets.

\smallskip\noindent\textbf{Binary $Q_{2^k-1}$.}
Two half-size children under one keyed glue factor; decode by peeling
against the known monic $H$, reading $\gamma$ at the leading row of the
remainder, and subtracting.  The height is at most $k$.
...
Place a $T_{k,8}$ pair inside the fill $A_4$ (level $l=2$); decode its top scalar/block
...
\begin{figure}[H]
  \centering
  \resizebox{\textwidth}{!}{\input{figures/fig_decoder_language}}
...
Along the recursive calls made for degree~$n$, every integer divided by
divides one of
\begin{equation}
\label{eq:bad-primes}
...
\begin{itemize}
  ...
  \item $2k$: the five crown slopes $2k,2k,-2k,k,k$ of the $4k+1$ branch
    (\Cref{lem:4k+1-splittable}), the slopes $1,2k,-2k,k,k$ of
    $Q_{4k+1}(x,H_2)$ (\Cref{lem:Q4k+1-from-H2}), and the slope $M=2k$ of a perturbed
...
known-powers gadgets $Q_{2^k-1}$, the gadgets
