...
\paragraph{Where the known powers come from.}
The powers $H_{2^i}$ are not a fixed family: they are the rungs of the power
tower that the $T$ recursion of \Cref{sec:splittable-4k+1} builds, one product
per rung, alongside the pair it computes.  Explicitly (\Cref{alg:constr-Tk2l,alg:constr-Tk2l-base}
and \Cref{fig:T-tower}),
\[
  H_2=x^2+bx+c,\qquad
...
and in general
\[
  H_{2D}=H_D^2-U_1^2+V_1\ \text{(even step)}\qquad\text{or}\qquad
  H_{2D}=(H_D+U_1)^2-V_1^2+W_1\ \text{(odd step)},
\]
where $U_1,V_1,W_1$ are lower powers of the same tower plus small
$Q$-blocks carrying fresh parameters ($b,c,a,e$ are outer parameters of the
degree-$(4k+1)$ family, \Cref{lem:4k+1-splittable}).  So every $H_{2^i}$ is
monic of degree $2^i$ by construction, and ``known'' means that it has been
built and decoded before any $Q$-block or fill uses it.  This section uses
nothing else about the powers: the decoders below take $H_2,\ldots,H_{2^{k-1}}$
as given data.
