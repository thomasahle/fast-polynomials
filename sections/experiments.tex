...
\texttt{tools/bench/\allowbreak carryless\_arm.cpp} and
\texttt{tools/bench/\allowbreak carryless.cpp};
\texttt{tools/bench/\allowbreak bench\_tabrows.cpp} times every row of the tabulation comparison, the non-polynomial hashes included, under the same wrapper)
...
\raggedright \texttt{smartcl\_64::\allowbreak mult3} in \texttt{tools/bench/\allowbreak framework/\allowbreak fast\_hashing\_arm.h} and \texttt{fast\_hashing.h} &
...
\raggedright \texttt{quartic2\_64} (Motzkin's two-multiplication quartic), \texttt{quartic3\_64} (the three-multiplication lift $x\,Q_3(x)+c_0$) and \texttt{motzkin\_61} (Motzkin's quartic over $2^{61}-1$) in \texttt{tools/bench/\allowbreak framework/\allowbreak fast\_hashing\_arm.h} and \texttt{fast\_hashing.h}; \texttt{mult4} is the first on ARM and the second on x86 &
...
\raggedright \texttt{tools/bench/\allowbreak appendix\_char2\_\allowbreak circuits\_arm.cpp} &
...
\raggedright \texttt{tools/bench/\allowbreak framework/\allowbreak injective\_hashing.h}; vector-resident rows: \texttt{tools/bench/\allowbreak adversarial/} &
...
\raggedright \texttt{tools/bench/\allowbreak chainhash/\allowbreak chainhash.h} &
...
The benchmark is implemented in \texttt{tools/bench/\allowbreak app\_\allowbreak countsketch\_\allowbreak arm.cpp} (ARM) and
\texttt{tools/bench/\allowbreak app\_\allowbreak countsketch\_\allowbreak x86.cpp} (x86).
...
\texttt{tools/bench/\allowbreak shamir\_\allowbreak sharegen\_\allowbreak mersenne.cpp}.
Including the cost of writing shares to memory gives essentially the same speedups; see
\texttt{tools/bench/\allowbreak shamir\_\allowbreak sharegen\_\allowbreak mersenne\_\allowbreak store.cpp}.
The x86 rows of the two tables below are the Xeon 8375C and the ARM rows
the Apple M2 Pro; the logs of these runs are in
\texttt{tools/bench/\allowbreak results/}.
