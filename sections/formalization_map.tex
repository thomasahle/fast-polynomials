...
\end{tabular}\par
\noindent\begin{tabular}{p{0.95in}p{2.05in}p{1.75in}p{0.55in}}
\raggedright \Cref{thm:construction-height-peeled} &\raggedright
...
\end{tabular}\par
\noindent\begin{tabular}{p{0.95in}p{2.05in}p{1.75in}p{0.55in}}
\raggedright \Cref{thm:construction-height} (sequential baseline) &\raggedright only the depth measure \texttt{Circuit.multDepth},
\texttt{MultiplicationProgram.height} (\texttt{Height/\allowbreak Depth.lean}) &
\raggedright The $\tfrac14L^2+L+2$ ledger for the sequential-fill schedules has no Lean
statement; the checked height is the peeled bound above. &
not formalized\\
\end{tabular}\par
\noindent\begin{tabular}{p{0.95in}p{2.05in}p{1.75in}p{0.55in}}
...
\end{tabular}\par
\noindent\begin{tabular}{p{0.95in}p{2.05in}p{1.75in}p{0.55in}}
\raggedright \Cref{alg:constr-known-2n-1}, \Cref{lem:Q-unitriangular} &\raggedright \texttt{mers}, \texttt{mers\_\allowbreak correct} (\texttt{Section4/\allowbreak KnownPowers.lean});
\texttt{mers\_\allowbreak unitriangular} (\texttt{Section4/\allowbreak MersCert.lean}) &
...
\raggedright Same interface as \texttt{mers\_\allowbreak correct}; exact counts
$2^{k-1}-1$ products, $5\cdot2^{k-2}-2$ additions, depth $k$. &
full\\
\end{tabular}\par
\noindent\begin{tabular}{p{0.95in}p{2.05in}p{1.75in}p{0.55in}}
...
\end{tabular}\par\nopagebreak
\noindent\begin{tabular}{p{0.95in}p{2.05in}p{1.75in}p{0.55in}}
\raggedright Lemma ($6$ parameters), \Cref{sec:lower} &\raggedright \texttt{no\_\allowbreak rationalInverse\_\allowbreak affine},
\texttt{no\_\allowbreak rationalInverse\_\allowbreak affine\_\allowbreak of\_\allowbreak ringChar\_\allowbreak ne\_\allowbreak two}
(\texttt{LowerBound/\allowbreak Main.lean}) &
\raggedright \texttt{[Field F] [Infinite F]}; \texttt{(2 : F) $\ne$ 0} (resp.\
\texttt{ringChar F $\ne$ 2}); six distinct points; a circuit in the displayed
normal form (\texttt{Circuit F}); any affine slot map $Mp+m_0$, no
invertibility of $M$.  Field hypotheses and conclusion match \Cref{lem:six-params}
and its proof in \Cref{appendix:lower}: an infinite field with $2\ne0$, and only a
left inverse is excluded.
The reduction of an arbitrary three-multiplication program to the normal
form is modelling (the definition of \texttt{Circuit}), not a theorem. &
full (normal form)\\
\end{tabular}\par
\noindent\begin{tabular}{p{0.95in}p{2.05in}p{1.75in}p{0.55in}}
...
\begin{sloppypar}
The construction, cost, height, example, and characteristic-two lower-bound
files (including \texttt{HeightFinal.lean} and
\texttt{LowerBoundChar2/\allowbreak General.lean}) are imported by the umbrella
module \texttt{FastPoly.lean}, the default target; \texttt{lake build FastPoly}
completes it (2006 jobs) with no errors.  The degree-$6$ lower bound
\texttt{LowerBound/\allowbreak } is a separate module tree built as an explicit
target.  No file under \texttt{FastPoly/\allowbreak } declares an \texttt{axiom} or
