...
abstract makes no formalization claim; the introduction's statement that the
complete upper-bound statement for every $n\ge3$---multiplication count,
decoder, and height---is formalized in Lean refers to the construction of
\Cref{thm:main} and \Cref{thm:construction-height}: the decoder,
...
\texttt{PaperMain.lean}.  The statement does not refer to independent
auxiliary results recorded later in the paper, whose coverage is reported
separately below.
The
...
\texttt{MainArrangementsChecked.\allowbreak of\_\allowbreak charZero},
\texttt{MainArrangementsChecked.\allowbreak of\_\allowbreak charP}
...
\texttt{MvPolynomial.\allowbreak algEquivOfDecodable} (\texttt{Automorphism.lean}) &
...
\Cref{lem:triangular-implies-compatible} &\raggedright \texttt{CoeffTriangular}, \texttt{CoeffTriangular.\allowbreak shift\_\allowbreak pivot},
\texttt{CoeffTriangular.\allowbreak toCompatiblePair} (\texttt{Recover/\allowbreak Triangular.lean}) &
...
\end{tabular}\par
\noindent\begin{tabular}{p{0.95in}p{2.05in}p{1.75in}p{0.55in}}
\raggedright \Cref{lem:polynomial-left-inverse-automorphism} &\raggedright \texttt{MvPolynomial.\allowbreak algEquivOfDecodable} (\texttt{Automorphism.lean});
...
\end{tabular}\par
\noindent\begin{tabular}{p{0.95in}p{2.05in}p{1.75in}p{0.55in}}
\raggedright Website characteristic-two constructions (degrees $5$--$24$; degrees $7$, $17$ and their even lifts assume a perfect field; the degree-$25$ inverse is in progress) &\raggedright
\texttt{construction}, \texttt{monic\_\allowbreak evaluation}
(\texttt{Examples/\allowbreak Char2Finite.lean}); odd bases in
\texttt{Char2Degree5/7/9/11/13/15.lean}. &
\raggedright Explicit coefficient decoders and literal circuits with
$\lfloor n/2\rfloor+1$ products; even degrees use a one-product lift.
Perfect characteristic-two fields for degrees $7,8$; arbitrary
characteristic-two fields for the other bases. Normalized-coordinate
evaluation is bijective; no general raw-key bijection is asserted here. &
full\\
\bottomrule
\end{tabular}\par
\endgroup
...
completes it with no errors.  The last verified full build of the umbrella
finished with 2003 jobs (2026-09-03), and the degree-six lower-bound targets
\texttt{FastPoly.\allowbreak LowerBound.\allowbreak General.\allowbreak Main} and
\texttt{FastPoly.\allowbreak LowerBound.\allowbreak General.\allowbreak Transport}
build with 1728 jobs (2026-09-05).  The build described here was verified at
The build described here was verified at commit \texttt{1aeab99} of the repository (\texttt{lake build FastPoly FastPoly.LowerBound.Main FastPoly.LowerBound.General.Main FastPoly.LowerBound.General.Transport}, 2156 jobs, 2026-09-06). No file under \texttt{FastPoly/\allowbreak } declares an \texttt{axiom} or
