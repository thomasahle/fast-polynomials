...
separately displayed alternatives of degrees $9$, $11$, and $13$.  The other
...
companion script \texttt{char2/verify\_n17\_uniform\_symbolic.py} (run from
the repository root as \texttt{python3 -m char2.verify\_n17\_uniform\_symbolic},
since it imports \texttt{char2/symexpr.py}) independently
...
later ones to zero, and collect the named row.  The companion script
\texttt{char2/verify\_n23\_unitriangular\_symbolic.py} (run from the
repository root as
\texttt{python3 -m char2.verify\_n23\_unitriangular\_symbolic}, since it
imports \texttt{tools/char2\_inverse\_finder.py}; its last wire $n$ is our
$f$) expands every identity exactly in $\F_2[q_0,\ldots,q_{22}][x]$: the
...
\texttt{char2/verify\_n25\_unitriangular\_symbolic.py} (run as
\texttt{python3 -m char2.verify\_n25\_unitriangular\_symbolic}; it imports
