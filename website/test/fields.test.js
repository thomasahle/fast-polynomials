...
// doubles, and every backend decodes + renders C for degrees 3..20 (char 2:
// the odd degrees it has circuits for). Plain node, exit 1 on failure.
...
// ---------- registry ----------
check(FIELD_IDS.join(',') === 'Q,R,p61,p89,p127,gf32,gf64,gf128', `registry ids: ${FIELD_IDS.join(',')}`);
...
for (const f of FIELDS) {
  ...
  check(typeof f.label === 'string' && !/\^/.test(f.label) && /<sup>|ℚ|ℝ/.test(f.labelHtml), `${f.id}: labels (${f.label} / ${f.labelHtml})`);
...
check(groupIds.join(',') === 'exact,mersenne,binary' && fieldById('Q').group === 'exact' && fieldById('R').group === 'exact', 'three groups: ℚ and ℝ share the exact group');
...
for (const f of FIELDS) {
  ...
  check(typeof r.cText === 'string' && r.cText.includes('eval_P'), `${tag}: C code`);
  ...
  check(f.id !== 'R' || (typeof r.maxRelError === 'number' && r.maxRelError < 1e-9), `${tag}: R error ${r.maxRelError}`);
  ...
  for (const c of r.comparisons) if (c.ok) check(typeof c.cText === 'string', `${tag}/${c.name}: C code`);
  if (f.id === 'R') {
...
// degrees with circuits, compileChar2 verifies by exact re-expansion.)
{
  ...
  const MUL = { Q: 'double eval_P(double x)', R: 'double eval_P(double x)', p61: 'mul61(', p89: 'mult_mod', p127: 'mul127(',
    ...
    for (const n of degrees) {
      ...
      try {
        if (f.lane === 'char2') {
          ...
          if (f.id === 'R') check(typeof r.maxRelError === 'number' && !/\d\/\d/.test(r.mathText), `${tag}: R diagnostics`);
          allC &&= typeof r.cText === 'string' && r.cText.includes('eval_P') && r.cText.includes(MUL[f.id]);
        }
