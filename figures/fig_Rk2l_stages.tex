...
\begin{tikzpicture}[x=1cm,y=1cm,>=Latex]
  ...
  \node[fp active, minimum width=16mm, anchor=west] (ou) at (0,3.22)
    {$u$\\$-k(k-1)$};
  \node[fp second, minimum width=13mm, anchor=west] (ov) at (1.6,3.22)
    {$v$\\$-(k-1)$};
  \node[fp pivot, minimum width=10mm, anchor=west] (ow) at (2.9,3.22)
    {$w$\\$m$};
  \node[fp pivot, minimum width=10mm, anchor=west] (oz) at (3.9,3.22)
    {$z$\\$m$};
  \node[fp recursive, minimum width=35mm, anchor=west] (orr) at (4.9,3.22)
    {inner pivots shifted by $4$\\through $F_1,F_2$};
  ...
  \node[fp pivot, minimum width=8mm, anchor=west] at (12.4,1.38)
    {$\alpha_0$\\$1$};
  \node[fp label, anchor=west] at (0,0.62)
    {Boundary corrections within the tail and at block junctions are given in the stage tables.};
  \draw[fp dependence] (zeta.south east) to[bend right=10] (rr.south west);
  \node[fp label, anchor=west, text=fpBlue!80!black] at (0,0.12)
    {tail pivots $\longrightarrow$ recursive pivots $\longrightarrow$ low unitriangular pivots (odd branches only)};
\end{tikzpicture}
