...
\begin{tikzpicture}[x=1cm,y=1cm,>=Latex]
  ...
  \node[fp pivot, minimum width=29mm] (q) at (f.center |- 0,3.08) {$q_t,q_{t-1},\ldots$};
  \draw[fp decode] (f.south) -- node[fp label, left=1.5mm] {leading $1$} (q.north);
  ...
  \node[fp pivot, minimum width=28mm] (proot) at (pow.center |- 0,3.08) {$p_{n-1},\ldots,p_0$};
  \draw[fp decode] (pow.south) -- node[fp label, left=1.5mm] {pivot $m$} (proot.north);
  ...
  \node[fp pivot, minimum width=11mm, anchor=west] (delta) at (9.75,1.08) {$\delta$};
  \node[fp second, minimum width=21mm, anchor=west] (low) at (10.85,1.08) {low remainder};
