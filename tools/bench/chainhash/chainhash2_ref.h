/* ***********************************************************************
 * ChainHash-2 (strided word pairing, candidate B1) -- SLOW REFERENCE
 * implementation (plain C++, no intrinsics).  Bit-by-bit carry-less
 * multiply, bit-by-bit reduction modulo x^64 + x^4 + x^3 + x + 1, a single
 * PH accumulator, byte-wise word assembly with implicit zero padding.
 *
 * Must compute exactly the same function as chainhash2.h:
 *   n = max(1, ceil(len / (8*BLOCK_WORDS))) blocks.  Blocks 1..n-1 are full
 *   (BLOCK_WORDS/2 pairs).  The last block holds r = len - (n-1)*
 *   8*BLOCK_WORDS bytes and only G' = ceil(r/32) GROUPS of 32 bytes =
 *   W' = 2G' pairs (W' = 0 for the empty message); its final partial group
 *   is zero-padded to 32 bytes.  PAIRING (the only difference from
 *   chainhash_ref.h): pair 2j of a block is (w_{4j}, w_{4j+2}), pair 2j+1 is
 *   (w_{4j+1}, w_{4j+3}), every word XORed with the key word of its own
 *   position, i.e. pair s = (w_{4(s/2) + s%2}, w_{4(s/2) + s%2 + 2}).
 ...
 *     alpha(s) = 4(s/2) + s%2,  beta(s) = alpha(s) + 2  (word indices within the block);
 *   the n*S pairs are fed to the recurrence in order (j, i);
 *   a_{n,S-1} ^= len (byte length, into the LAST pair);
 *   P_0 = z,  P_m = a_m + (b_m + y)(P_{m-1} + u)
 *   v = P_{nS} + t_in   (INTEGER 64-bit addition mod 2^64: the input twist)
 *   out = chain_5(v)    (CIRCUITS[5] of website/js/char2.js, 3 multiplications).
 * Key: k[0..BLOCK_WORDS), u, y, z, c[0..5), t_in  (BLOCK_WORDS + 9 words),
 * derived from the seed in that order (identical to chainhash_ref.h).
 * *********************************************************************** */

#ifndef CHAINHASH2_REF_H
#define CHAINHASH2_REF_H

#include <cstddef>
#include <cstdint>

namespace chainhash2_ref {
...
/* Degree-5 chain, same gate order as the fast version (char2.js CIRCUITS[5]). */
template <int K>
static inline uint64_t chain(const uint64_t* c, uint64_t v) {
    static_assert(valid_degree(K), "chain: only K = 5 is shipped");
    uint64_t y = gfmul(v, v);
    uint64_t z = gfmul(y ^ c[0], v ^ y ^ c[1]);
    uint64_t t = gfmul(v ^ c[2], z ^ c[3]);
    return t ^ c[4];
}
...
static inline uint64_t hash(const Key<BLOCK_WORDS, K, S>& key, const void* data, size_t len) {
    ...
    for (size_t j = 0; j < n; j++) {
        const size_t r = (j + 1 == n) ? len - j * BB : BB;  // bytes in this block (r = 0 for the empty message)
    ...
    return chain<K>(key.c, P + key.t_in);
}

}  // namespace chainhash2_ref

#endif  // CHAINHASH2_REF_H
