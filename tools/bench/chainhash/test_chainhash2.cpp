/* Tests for ChainHash-2 (tools/bench/chainhash/chainhash2.h: the STRIDED
 * word pairing candidate, item B1) against its bit-serial reference
 * (chainhash2_ref.h).  Structure copied from test_chainhash.cpp: the same
 * grid, the same length set L, T1-T7 with the same meaning, plus
 *
 *  T8  cross-check against the SHIPPED function (chainhash.h): for every
 *      length that is a multiple of 32 bytes (0, 32, ..., 4096, and the
 *      lengths of L that are multiples of 32), chainhash2(k, m) ==
 *      chainhash(pi k, pi m) where pi swaps the two middle words of every
 *      32-byte group (the two functions have the same pair count 2 ceil(r/32)
 *      = ceil(r/16) there and differ only by the fixed permutation of word
 *      positions applied to data and key alike).  Grid points (8,5,1),
 *      (32,5,1), (32,5,2), (128,5,2), 20 keys each.
 *
 * The original header comment of test_chainhash.cpp (T1-T7) follows.
 */
...
#include "chainhash2.h"
#include "chainhash.h"  // T8: cross-check against the shipped function on 32-byte-multiple lengths
#include "chainhash2_ref.h"
...
static long test_fast_vs_ref(int n_keys) {
    using FKey = chainhash2::Key<BW, K, S>;
    using RKey = chainhash2_ref::Key<BW, K, S>;
    long count = 0;
    for (int ki = 0; ki < n_keys; ki++) {
        ...
        for (size_t li = 0; li < g_lengths.size(); li++) {
            ...
            uint64_t hf = chainhash2::hash(fk, m, len);
            uint64_t hr = chainhash2_ref::hash(rk, m, len);
...
static long test_determinism(int n_seeds) {
    ...
    for (int i = 0; i < n_seeds; i++) {
        ...
        uint64_t h1 = chainhash2::hash(a, buf.base, len);
        uint64_t h2 = chainhash2::hash(b, buf.base, len);
        std::vector<uint8_t> copy(buf.base, buf.base + len);
        uint64_t h3 = chainhash2::hash(a, copy.data(), len);
...
static long test_length_sensitivity(int n_keys, int msgs_per_key) {
    ...
    for (int ki = 0; ki < n_keys; ki++) {
        ...
        for (int mi = 0; mi < msgs_per_key; mi++) {
            ...
            m[len] = 0;
            m[len + 1] = 0;
            uint64_t h0 = chainhash2::hash(fk, m.data(), len);
            uint64_t h1 = chainhash2::hash(fk, m.data(), len + 1);
            uint64_t h2 = chainhash2::hash(fk, m.data(), len + 2);
...
static long test_guard_page() {
    ...
    for (size_t li = 0; li < g_lengths.size(); li++) {
        ...
        if (len == 0) {  // pointer AT the guard page with len 0 must not be touched
            uint64_t hg = chainhash2::hash(fk, guard, 0);
        ...
        hf = chainhash2::hash(fk, data, len);
        hr = chainhash2_ref::hash(rk, data, len);
...
static Poly pmul(const Poly& a, const Poly& b) {
    ...
    for (size_t i = 0; i < a.size(); i++)
        for (size_t j = 0; j < b.size(); j++) r[i + j] ^= chainhash2_ref::gfmul(a[i], b[j]);
    return r;
}
...
static uint64_t horner(const Poly& p, uint64_t v) {
    uint64_t h = 0;
    for (size_t i = p.size(); i-- > 0;) h = chainhash2_ref::gfmul(h, v) ^ p[i];
    return h;
}
...
static long test_chain_is_monic_poly(int n_params, int n_points) {
    using FKey = chainhash2::Key<BW, K, 1>;
    using RKey = chainhash2_ref::Key<BW, K, 1>;
    long count = 0;
    for (int i = 0; i < n_params; i++) {
        ...
        for (int j = 0; j < n_points; j++) {
            ...
            uint64_t hf = chainhash2::chain<K>(fk.c, v);
            uint64_t hr = chainhash2_ref::chain<K>(fk.c, v);
            ...
            uint64_t tf = chainhash2::finalize(fk, w);
            uint64_t tr = chainhash2_ref::finalize(rk, w);
...
static long test_collisions_pairs(long n_pairs) {
    ...
    for (long i = 0; i < n_pairs; i++) {
        ...
        if (chainhash2::hash(fk, m1, l1) == chainhash2::hash(fk, m2, l2)) collisions++;
    }
...
static long test_collisions_set(long n_msgs) {
    ...
    for (long i = 0; i < n_msgs; i++) {
        ...
        hs.push_back(chainhash2::hash(fk, m, len));
    }
...
static long test_gfmul(long n) {
    ...
    for (uint64_t a : edge)
        for (uint64_t b : edge) {
            CHECK(chainhash2::gfmul(a, b) == chainhash2_ref::gfmul(a, b), "gfmul(%llx,%llx)", (unsigned long long)a,
                  (unsigned long long)b);
            count++;
        }
    for (long i = 0; i < n; i++) {
        uint64_t a = rng(), b = rng();
        uint64_t f = chainhash2::gfmul(a, b), r = chainhash2_ref::gfmul(a, b);
    ...
    for (long i = 0; i < 1000; i++) {
        ...
        CHECK(chainhash2::gfmul(a, b) == chainhash2::gfmul(b, a), "gfmul not commutative");
        CHECK(chainhash2::gfmul(chainhash2::gfmul(a, b), c) == chainhash2::gfmul(a, chainhash2::gfmul(b, c)),
...
/* ------------------------------------------------------------------ */
/* T8: chainhash2(k, m) == chainhash(pi k, pi m) for 32 | len            */
/* ------------------------------------------------------------------ */
...
template <int BW, int K, int S>
static long test_vs_shipped(int n_keys) {
    using KN = chainhash2::Key<BW, K, S>;
    using KO = chainhash::Key<BW, K, S>;
    ...
    for (int ki = 0; ki < n_keys; ki++) {
        uint64_t seed = rng();
        KN kn = KN::from_seed(seed);
        KO ko = KO::from_seed(seed);
        for (int g = 0; g + 4 <= BW; g += 4) std::swap(ko.k[g + 1], ko.k[g + 2]);   // pi k
        ko.setup();
        CHECK(kn.u == ko.u && kn.y == ko.y && kn.z == ko.z && kn.t_in == ko.t_in && std::memcmp(kn.c, ko.c, sizeof(kn.c)) == 0,
              "T8 BW=%d S=%d: non-PH key words differ", BW, S);
        for (size_t len : lens) {
            size_t off = rng() % 16;
            ExactBuf buf(len + off), buf2(len + off);
            fill_random(buf.base, len + off);
            permute_words32(buf2.base + off, buf.base + off, len);   // pi m
            uint64_t hn = chainhash2::hash(kn, buf.base + off, len);
            uint64_t ho = chainhash::hash(ko, buf2.base + off, len);
            CHECK(hn == ho, "T8 BW=%d S=%d key %d len %zu: chainhash2 %016llx chainhash(pi) %016llx", BW, S, ki, len,
...
int main(int argc, char** argv) {
    ...
    build_lengths();
    std::printf("ChainHash-2 (strided pairing candidate) tests: K = 5 (char2.js CIRCUITS[5]) behind the additive input twist; grid (8,5,1), "
                ...
                "twist = integer add:\n");
    {
        ...
        std::printf("  K= 5 (%d mults): monic degree-5, fast == ref == Horner: %ld points (200 parameter sets; each "
                    ...
                    chainhash2::chain_mults(5), n5, b5);
    }

    {
        long n8 = test_vs_shipped<8, 5, 1>(20) + test_vs_shipped<32, 5, 1>(20) + test_vs_shipped<32, 5, 2>(20) +
                  test_vs_shipped<128, 5, 2>(20);
        std::printf("T8 chainhash2(k, m) == chainhash(pi k, pi m) for 32 | len: %ld messages over (8,5,1), (32,5,1), (32,5,2), (128,5,2)\n", n8);
    }
