#!/bin/bash
# Lane E step 3 ("final6": length XORed into both halves of the last pair), M2 Pro:
# harness timing then SMHasher3 full suites (sequential), each behind a 1-min-load < 3 gate (20 min max).
...
ln -sf ../bytes1.bin bytes1.bin 2>/dev/null
waitload
echo "$(date +%T) harness start load1=$(loadnow) uptime: $(uptime)" >> $E/m2_load.log
./speed 5 0.5 run ChainHash > $E/final6_m2_harness_chainhash.txt 2>&1
echo "$(date +%T) harness end exit=$? load1=$(loadnow)" >> $E/m2_load.log
...
for h in chainhash-256 chainhash-1k; do
  ...
  ./SMHasher3 $h > $E/final6_m2_${h#chainhash-}.txt 2>&1
