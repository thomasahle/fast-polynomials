/* ***********************************************
 * Injective/Universal Hashing for ARM NEON
 *
 ...
 *
 * Compile: clang++ -O3 -std=c++17 -march=armv8-a+crypto
 * ***********************************************/

#ifndef _INJECTIVE_HASHING_ARM_H_
#define _INJECTIVE_HASHING_ARM_H_
...
public:
    ...
    hash128_t operator()(uint64_t x) {
        ...
        for (size_t i = 0; i < N; i++) {
            ...
            uint8x16_t prod = poly_to_u8(clmul_lo_lo(
...
static inline void clmul_128x128(uint64_t a_lo, uint64_t a_hi,
    ...
    uint8x16_t z0 = poly_to_u8(clmul_lo_lo(from64(a_lo), from64(b_lo)));
    uint8x16_t z2 = poly_to_u8(clmul_lo_lo(from64(a_hi), from64(b_hi)));
    uint8x16_t z1 = poly_to_u8(clmul_lo_lo(from64(a_lo ^ a_hi), from64(b_lo ^ b_hi)));
...
static inline void reduce_gf128(uint64_t r0, uint64_t r1, uint64_t r2, uint64_t r3,
    ...
    uint8x16_t r3_red = poly_to_u8(clmul_lo_lo(from64(r3), from64(0x87)));
    r1 ^= lower64(r3_red);
    r2 ^= upper64(r3_red);

    // Reduce r2 (bits 128-191): multiply by reduction polynomial
    uint8x16_t r2_red = poly_to_u8(clmul_lo_lo(from64(r2), from64(0x87)));
    r0 ^= lower64(r2_red);
    r1 ^= upper64(r2_red);
...
#endif  // _INJECTIVE_HASHING_ARM_H_
