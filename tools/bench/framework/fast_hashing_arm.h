 ...
 * is exactly 7-wise independent.  (smartcl_64<7>::mult7 above is the search
 * circuit, u = (x + y + t + k4)(x + k5), for which no inverse is displayed.)
 * ***************************************************/
...
public:
    void init() {
        for (int i = 0; i < 7; i++) k[i] = getRandomUInt64();
    }
    void set_keys(const uint64_t c[7]) {
        for (int i = 0; i < 7; i++) k[i] = c[i];
    }
    uint64_t operator()(uint64_t x) {
        uint64_t y = slemul(x, x ^ k[0]);               // y = x (x + c0)
        uint64_t z = slemul(x ^ k[1], y ^ k[2]);        // z = (x + c1) (y + c2)
        uint64_t t = slemul(z, z ^ k[3]);               // t = z (z + c3)
        uint64_t u = slemul(x ^ k[4], y ^ t ^ k[5]);    // u = (x + c4) (y + t + c5)
        return u ^ k[6];                                // P = u + c6
    }
};
