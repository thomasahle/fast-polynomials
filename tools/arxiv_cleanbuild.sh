...
for i in 1 2 3; do pdflatex -interaction=nonstopmode -halt-on-error main.tex > pass$i.log 2>&1 || { echo "PASS $i FAILED"; grep -n "^!" -A3 main.log | head -20; exit 1; }; done
...
echo "missing files: $(grep -c 'No file\|not found' main.log)"; grep -n "No file\|not found" main.log | head -5
