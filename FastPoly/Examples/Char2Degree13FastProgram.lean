...
/-! The original seven-product circuit, with separately named gate/bind equations.
Gate syntax is copied literally from the existing active degree-thirteen circuit.
Only one gate or one bind is evaluated in each proof. -/
...
def gate1 : Cost.Circuit F (Inputs 1) 1 :=
  .mul (.add (.add (.input (Sum.inl (0))) (.input (Sum.inr 0))) (.input (Sum.inl (13)))) (.add (.input (Sum.inr 0)) (.input (Sum.inl (12))))

def gate2 : Cost.Circuit F (Inputs 2) 1 :=
  .mul (.add (.add (.input (Sum.inl (Sum.inr 0))) (.input (Sum.inr 0))) (.input (Sum.inl (Sum.inl (11))))) (.add (.input (Sum.inr 0)) (.input (Sum.inl (Sum.inl (10)))))

def gate3 : Cost.Circuit F (Inputs 3) 1 :=
  .mul (.add (.add (.input (Sum.inl (Sum.inl (Sum.inr 0)))) (.input (Sum.inl (Sum.inr 0)))) (.input (Sum.inl (Sum.inl (Sum.inl (9)))))) (.add (.input (Sum.inr 0)) (.input (Sum.inl (Sum.inl (Sum.inl (8))))))

def gate4 : Cost.Circuit F (Inputs 4) 1 :=
  .mul (.add (.add (.input (Sum.inl (Sum.inl (Sum.inr 0)))) (.input (Sum.inr 0))) (.input (Sum.inl (Sum.inl (Sum.inl (Sum.inl (7))))))) (.add (.input (Sum.inl (Sum.inl (Sum.inl (Sum.inl (0)))))) (.input (Sum.inl (Sum.inl (Sum.inl (Sum.inl (6)))))))

def gate5 : Cost.Circuit F (Inputs 5) 1 :=
  .mul (.add (.add (.input (Sum.inl (Sum.inl (Sum.inl (Sum.inl (Sum.inl (0))))))) (.input (Sum.inl (Sum.inl (Sum.inl (Sum.inl (Sum.inr 0))))))) (.input (Sum.inl (Sum.inl (Sum.inl (Sum.inl (Sum.inl (5)))))))) (.add (.input (Sum.inl (Sum.inl (Sum.inl (Sum.inl (Sum.inl (0))))))) (.input (Sum.inl (Sum.inl (Sum.inl (Sum.inl (Sum.inl (4))))))))

def gate6 : Cost.Circuit F (Inputs 6) 1 :=
  .mul (.add (.add (.input (Sum.inl (Sum.inl (Sum.inl (Sum.inr 0))))) (.input (Sum.inr 0))) (.input (Sum.inl (Sum.inl (Sum.inl (Sum.inl (Sum.inl (Sum.inl (3))))))))) (.add (.input (Sum.inl (Sum.inl (Sum.inl (Sum.inl (Sum.inl (Sum.inr 0))))))) (.input (Sum.inl (Sum.inl (Sum.inl (Sum.inl (Sum.inl (Sum.inl (2)))))))))

def tail7 : Cost.Circuit F (Inputs 7) 1 :=
  (.add (.add (.add (.input (Sum.inl (Sum.inl (Sum.inl (Sum.inr 0))))) (.input (Sum.inl (Sum.inl (Sum.inr 0))))) (.input (Sum.inr 0))) (.input (Sum.inl (Sum.inl (Sum.inl (Sum.inl (Sum.inl (Sum.inl (Sum.inl (1))))))))))
...
theorem multiplication_count : (circuit (F := F)).gates.multiplications = 7 := by
  simp only [circuit, tail0, tail1, tail2, tail3, tail4, tail5, tail6, tail7,
    gate0, gate1, gate2, gate3, gate4, gate5, gate6, Cost.Circuit.gates, Cost.Circuit.gates_input,
...
def program : Cost.MultiplicationProgram F ℕ 1 7 :=
...
theorem gate5_eval (q : Keys F) (i : Fin 1) :
    (gate5 (F := F)).eval (env5 q) i = t q := by
...
theorem tail5_eval (q : Keys F) :
  ...
  have hg : (gate5 (F := F)).eval (env5 q) = (fun _ => t q) :=
    funext (gate5_eval q)
...
theorem tail4_eval (q : Keys F) :
  ...
  have hg : (gate4 (F := F)).eval (env4 q) = (fun _ => u q) :=
    funext (gate4_eval q)
...
theorem tail3_eval (q : Keys F) :
  ...
  have hg : (gate3 (F := F)).eval (env3 q) = (fun _ => v q) :=
    funext (gate3_eval q)
...
theorem tail2_eval (q : Keys F) :
  ...
  have hg : (gate2 (F := F)).eval (env2 q) = (fun _ => w q) :=
    funext (gate2_eval q)
...
/-- The original online ledger computes the exact named decoder polynomial. -/
